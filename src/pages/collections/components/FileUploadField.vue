    ...
    >
      ...
      <div v-if="!currentFiles.length" class="text-center">
        ...
        <p class="text-sm text-gray-600 mb-4">
          Arrastra y suelta
          {{ isMultiple ? "archivos aquí" : "un archivo aquí" }} o haz clic para
          seleccionar
        </p>
        <button
          type="button"
...
function handleFileSelect(event: Event) {
  ...
  if (target.files && target.files.length > 0) {
    ...
    if (
      props.isMultiple &&
      currentFiles.value.length + target.files.length > 10
    ) {
    ...
    for (let i = 0; i < target.files.length; i++) {
      if (target.files[i].size > 10 * 1024 * 1024) {
        alert(
          `El archivo "${target.files[i].name}" es muy grande. Máximo 10MB por archivo.`
        );
        return;
      }
    }
  ...
  if (files && files.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].size > 10 * 1024 * 1024) {
        alert(
          `El archivo "${files[i].name}" es muy grande. Máximo 10MB por archivo.`
        );
        return;
      }
    }
