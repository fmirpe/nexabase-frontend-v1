<template>
  <div class="space-y-4">
    ...
    >
        ...
        <div class="text-center">
          <div class="w-16 h-16 mx-auto mb-4 relative">
            <svg class="w-16 h-16 transform -rotate-90" viewBox="0 0 64 64">
              <circle cx="32" cy="32" r="28" stroke-width="4" stroke="#e5e7eb" fill="transparent" />
              <circle
                cx="32" cy="32" r="28" stroke-width="4" stroke="#10b981" fill="transparent"
          ...
          <p class="text-sm font-medium text-gray-700">Subiendo imagen{{ isMultiple ? 'es' : '' }}...</p>
        </div>
      </div>
      ...
      <div v-if="!currentImages.length" class="text-center">
        ...
        <p class="text-sm text-gray-600 mb-4">
          Arrastra y suelta {{ isMultiple ? 'imágenes aquí' : 'una imagen aquí' }} o haz clic para seleccionar
        </p>
        <button
          type="button"
        ...
        <p class="text-xs text-gray-500 mt-2">JPG, PNG, GIF, WebP • Máximo 5MB por imagen</p>
      </div>
      ...
      <div v-else class="space-y-4">
        ...
        <!-- Single Image -->
        <div v-if="!isMultiple && currentImages.length === 1" class="relative group">
          ...
          <div class="mt-2 text-center">
            <p class="text-sm font-medium text-gray-900 truncate">{{ getImageName(currentImages[0]) }}</p>
            <p class="text-xs text-gray-500">{{ getImageSize(currentImages[0]) }}</p>
          </div>
        </div>

        <!-- Multiple Images Grid -->
        <div v-else-if="isMultiple" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          <div
          ...
          >
            ...
            <p class="text-xs text-gray-600 mt-1 truncate text-center">{{ getImageName(image) }}</p>
          </div>
        </div>
      </div>
    ...
    <!-- Image Info -->
    <div v-if="currentImages.length > 0" class="text-xs text-gray-500 bg-gray-50 rounded-lg p-3">
...
function handleFileSelect(event: Event) {
  ...
  if (target.files && target.files.length > 0) {
    ...
    if (props.isMultiple && currentImages.value.length + target.files.length > 8) {
    ...
    for (let i = 0; i < target.files.length; i++) {
      if (target.files[i].size > 5 * 1024 * 1024) {
        alert(`La imagen "${target.files[i].name}" es muy grande. Máximo 5MB por imagen.`)
        return
      }
    }
  ...
  if (files && files.length > 0) {
    ...
    for (let i = 0; i < files.length; i++) {
      if (files[i].size > 5 * 1024 * 1024) {
        alert(`La imagen "${files[i].name}" es muy grande. Máximo 5MB por imagen.`)
        return
      }
    }
...
function getImageUrl(image: any): string {
  if (typeof image === 'string') {
    return `/api/storage/files/${image}`
  }
...
function getImageName(image: any): string {
  ...
  return image?.name || image?.filename || 'Imagen sin nombre'
}
