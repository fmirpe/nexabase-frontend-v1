      ...
      <table class="min-w-full divide-y divide-gray-200">
        <thead class="bg-gray-50">
          <tr>
            ...
            <!-- Headers dinámicos basados en el schema -->
            <th
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <span v-if="isRelationField(String(fieldName))" class="text-xs text-purple-500">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-if="records.length === 0">
            <td :colspan="Object.keys(collection.schema?.fields || {}).length + 3" class="px-6 py-8 text-center text-gray-500">
              {{ hasActiveFilters ? 'No se encontraron registros que coincidan con los filtros.' : 'No hay registros aún.' }}
            </td>
          </tr>
            ...
            >
              ...
              <div v-if="fieldDef.type === 'image'" class="flex items-center">
                <img
                  ...
                  :src="getFileUrl(record[String(fieldName)])"
              ...
              <!-- ✅ Campo File -->
              <div v-else-if="fieldDef.type === 'file'" class="flex items-center">
                <div v-if="record[String(fieldName)]" class="flex items-center gap-2">
                  ...
                  <div>
                    <a
                      :href="getFileUrl(record[String(fieldName)])"
                      target="_blank"
                      class="text-sm text-blue-600 hover:text-blue-800 font-medium"
                    >
                      Ver archivo
                    </a>
                    <p class="text-xs text-gray-500">{{ getFileName(record[String(fieldName)]) }}</p>
                  </div>
                </div>
                <span v-else class="text-gray-400 text-sm">Sin archivo</span>
              </div>
              
              <!-- ✅ Campo Images (múltiples) -->
              <div v-else-if="fieldDef.type === 'images'" class="flex items-center">
                <div v-if="record[String(fieldName)] && record[String(fieldName)].length > 0" class="flex -space-x-2">
                  <img
                    v-for="(imageId, index) in record[String(fieldName)].slice(0, 3)"
                    :key="imageId"
                  ...
                  />
                  <div
                    v-if="record[String(fieldName)].length > 3"
                    class="w-8 h-8 bg-gray-100 rounded-full border-2 border-white flex items-center justify-center"
                  >
                    <span class="text-xs text-gray-600">+{{ record[String(fieldName)].length - 3 }}</span>
                  </div>
                </div>
                <span v-else class="text-gray-400 text-sm">Sin imágenes</span>
              </div>
              
              <!-- ✅ Campo Files (múltiples) -->
              <div v-else-if="fieldDef.type === 'files'" class="flex items-center">
                <div v-if="record[String(fieldName)] && record[String(fieldName)].length > 0" class="flex items-center gap-1">
                  <div class="w-6 h-6 bg-blue-100 rounded flex items-center justify-center">
                    <svg class="w-3 h-3 text-blue-600" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M4 3a2 2 0 00-2 2v1.5h16V5a2 2 0 00-2-2H4zM2 8.5V17a2 2 0 002 2h12a2 2 0 002-2V8.5H2z" />
                    </svg>
                  </div>
                  <span class="text-sm text-gray-600">{{ record[String(fieldName)].length }} archivos</span>
                </div>
              ...
              <div v-else-if="fieldDef.type === 'json'" class="max-w-xs">
                <div v-if="record[String(fieldName)]" class="bg-gray-50 rounded px-2 py-1 text-xs font-mono text-gray-600 truncate">
              ...
              <!-- ✅ Campo Date -->
              <span v-else-if="fieldDef.type === 'date'" class="text-sm text-gray-900">
                {{ formatDate(record[String(fieldName)]) }}
              </span>
              
              <!-- ✅ Campo Text largo -->
              <div v-else-if="fieldDef.type === 'text'" class="max-w-xs">
                <p class="text-sm text-gray-900 truncate" :title="record[String(fieldName)]">
                  {{ record[String(fieldName)] || '—' }}
                </p>
              </div>
              ...
              <!-- ✅ Campo Email -->
              <a
                v-else-if="fieldDef.type === 'email' && record[String(fieldName)]"
              ...
              <!-- ✅ Campo Relation (belongs_to) -->
              <div v-else-if="isRelationField(String(fieldName))">
                <span v-if="record[String(fieldName)]" class="text-sm text-gray-900">
                  <div class="flex items-center gap-2">
                    ...
                    <div>
                      <p class="text-sm font-medium text-gray-900">
                        {{ getRelationDisplayValue(record[String(fieldName)], String(fieldName)) }}
                      </p>
              ...
              <!-- ✅ Otros campos (string, number, etc.) -->
...
function getRelationDisplayValue(relationValue: any, fieldName: string): string {
  ...
  // Si es un objeto expandido (con datos de la relación)
