<template>
  <div
    class="h-screen flex overflow-hidden bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900"
  >
    ...
    >
      ...
      <div class="absolute inset-0 pointer-events-none">
        ...
        <div v-if="!isNightTime" class="absolute inset-0">
          <div
            class="absolute top-20 left-10 w-32 h-16 bg-white/5 rounded-full blur-xl animate-float"
          ></div>
          <div
            class="absolute top-32 right-16 w-24 h-12 bg-white/3 rounded-full blur-lg animate-float-delay"
          ></div>
        </div>
      ...
      <div class="relative z-10 text-center max-w-md">
        ...
        <div class="mb-8 flex justify-center">
          <div class="relative">
            <!-- <div
              class="absolute inset-0 bg-white/20 rounded-2xl blur-xl transform scale-110"
            ></div> -->
            <div class="relative">
              <img
                v-if="!showFallback"
                :src="logoUrl"
                alt="NexaBase Logo"
                class="h-52 w-62 object-contain drop-shadow-2xl"
                @error="showFallback = true"
              />
              <svg
                v-else
                class="w-26 h-26 text-white drop-shadow-2xl"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path
        ...
        <div class="space-y-4">
          <div>
            <h1
              class="text-3xl font-bold mb-2 bg-gradient-to-r from-white via-blue-100 to-purple-100 bg-clip-text text-transparent"
            >
              NexaBase Dashboard
            </h1>
            <p class="text-lg text-white/80">Professional BaaS Management</p>
          </div>

          <!-- Feature highlights -->
          <div class="space-y-2 mt-6">
            <div
              class="flex items-center justify-center space-x-3 text-white/70 text-sm"
            >
              <div
            ...
            >
              <div
                class="w-2 h-2 bg-blue-400 rounded-full animate-pulse delay-300"
              ></div>
            ...
            >
              <div
                class="w-2 h-2 bg-purple-400 rounded-full animate-pulse delay-600"
              ></div>
        ...
        <div class="mt-8 pt-6 border-t border-white/10">
          <p class="text-xs text-black/60">
            © 2025 NexaBase v2.1.0 •
            <a
              href="https://fabricadealgoritmos.site"
              class="text-black hover:text-blue-100 underline transition-colors"
              target="_blank"
            >
    ...
    <div
      class="w-full lg:w-1/2 flex items-center justify-center p-4 bg-gradient-to-br from-gray-50 to-white relative overflow-hidden"
    >
      <!-- Background pattern -->
      ...
      <div class="w-full max-w-md px-4 relative z-10">
        ...
        <div class="lg:hidden text-center mb-4">
          <div class="mb-3 flex justify-center">
            <div class="relative">
              <div
                class="absolute inset-0 bg-indigo-500/20 rounded-xl blur-lg transform scale-110"
              ></div>
              <div
                class="relative bg-gradient-to-br from-indigo-500 to-purple-600 p-3 rounded-xl shadow-lg"
              >
                <img
                  v-if="!showFallback"
                  :src="logoUrl"
        ...
        <div
          class="bg-white/80 backdrop-blur-xl rounded-2xl shadow-xl p-6 border border-gray-200/50 relative overflow-hidden"
        >
          <!-- Decorative elements -->
          <div
            class="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-indigo-500 via-purple-500 to-blue-500"
          ></div>
...
const isNightTime = computed(
  () => currentHour.value >= 19 || currentHour.value <= 6
);
...
const dynamicBackground = computed(() => {
  if (isNightTime.value) {
    ...
    return {
      background:
        "radial-gradient(100% 100% at 50% 0%, #1a1d29 0%, #2d3142 45%, #4f5d75 100%)",
    };
  } else {
    // Día: gradiente más claro con tonos cálidos
    return {
      background:
        "radial-gradient(100% 100% at 50% 0%, #4a90e2 0%, #7cb9e8 45%, #b8d4f0 100%)",
    };
  }
});
...
const timeOverlay = computed(() => {
  if (isNightTime.value) {
    ...
    return {
      background:
        "linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(255,255,255,0.05) 40%, rgba(255,255,255,0) 80%)",
    };
  }
});
